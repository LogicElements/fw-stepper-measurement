...
Status_t MbRtu_WriteHoldingRegCallback(uint16_t address, uint16_t value) {
	...
	case MB_HOLD_COM_MB_TIMEOUT:
		...
		break;
	case MB_HOLD_RTD_MODE:
		//conf.stpmeas.mode = (rtd_mode_t) value;
		//id = CONF_RTD_MODE;
		break;
	case MB_HOLD_RTD_TEMP_CALIB:
		//conf.stpmeas.temp_calib = (rtd_temp_calib_t) value;
	...
	case MB_HOLD_RTD_NTC_BETA:
		...
		break;
	case MB_HOLD_RTD_NTC_STOCK_RES:
		conf.stpmeas.ntc_stock_res = value;
		//id = CONF_RTD_NTC_STOCK_RES;
		break;
	case MB_HOLD_RTD_PT_STOCK_RES:
		conf.stpmeas.pt_stock_res = value;
